...
const Game: React.FC = () => {
  ...
  useEffect(() => {
    if (!search.trim()) {         // Search button is empty 
      setSearchResults([]);
    ...
    const t = setTimeout(() => {
      axios
        .get(`${API}/game/search?s=${encodeURIComponent(search.trim())}`)
        .then(res => {
  ...
  return (
    <div className="games-container">
      ...
      <div className="search-container" style={{ position: "relative" }}>
        ...
        />
        <button type="button" onClick={handleSearchClick}><i className="fa fa-search"></i></button>
        ...
        {showDropdown && (
          <div className="search-dropdown">
            ...
            {!loading && searchResults.map((g: any) => (
              <div
                key={g.id}
          ...
          <div key={game.id} onClick={() => navigate(`/game/${game.id}`)} className="game-card">
            ...
            <div className="game-info">
              ...
              <button className="shopping-button"
                onClick={(e) => {
                  e.stopPropagation();
                  addToCart(game.id, game.name, game.price);}}
